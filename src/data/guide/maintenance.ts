...
export const MAINTENANCE_SECTIONS: MaintenanceSection[] = [
  ...
  },
  {
    ...
    tipsKeys: [
      ...
      { key: "maintenance.spending3", default: "If you need to speed up a transaction, both fee-bumping methods can reveal change. RBF can only be applied by the sender and reveals which output is change (the output whose value decreases in the replacement). A CPFP by the receiver does not reveal your change, but a CPFP by the sender spends the change output and reveals it too. For privacy-sensitive payments, set an adequate fee upfront to avoid needing either." },
