...
import { COLORS, HUES, V2_LIGHT_PALETTE, hexToRgba } from "@/lib/palette";
...
export function LensStage({ tx, model, L, lens, revealAll, reduced, selected, onPick }: Props) {
  ...
  const accent = model.blind ? GREY : P.bitcoinText;
  ...
  const outs: Card[] = tx.vout.map((vo, i) => {
    ...
    if (r?.kind === "payment") return { ...base, tag: t("v2.home.lens_payment_likely", { defaultValue: "LIKELY PAYMENT" }), main: t("v2.home.lens_paid_likely", { defaultValue: "Likely paid {{amount}}", amount: amt(vo.value) }), sub: r.entityName ?? t("v2.home.lens_payment_sub", { defaultValue: "the output that is not change" }), color: P.severityHigh };
  ...
  return (
    <svg viewBox={`0 0 ${L.W} ${L.H}`} className="block w-full h-auto select-none" aria-hidden="true">
      <defs>
        ...
        <pattern id="lens-scan" width="4" height="4" patternUnits="userSpaceOnUse"><rect width="4" height="1" fill={hexToRgba(P.background, 0.25)} /></pattern>
      ...
      {/* Wallet view: calm */}
      <rect width={L.W} height={L.H} fill={P.surface1} />
      ...
      <circle cx={L.hub.x} cy={L.hub.y} r={L.port ? 12 : 22} fill={P.surface2} stroke={FG(0.16)} />
      <rect x={b.x} y={b.y} width={b.w} height={b.h} rx={10} fill={P.surface1} stroke={FG(0.1)} strokeDasharray="3 4" />
      <text x={b.x + 14} y={b.y + 23} className="font-mono" fontSize={10.5} letterSpacing="0.08em" fill={P.faint}>{t("v2.home.lens_tx", { defaultValue: "TRANSACTION" })}</text>
      {[[t("v2.home.lens_fee", { defaultValue: "fee" }), `${fmtN(tx.fee)} sats`], [t("v2.home.lens_size", { defaultValue: "size" }), `${fmtN(tx.weight)} WU`], [t("v2.home.lens_block", { defaultValue: "block" }), tx.status.block_height ? fmtN(tx.status.block_height) : "-"]].map(([k, v], i) => (
        <g key={k} className="font-mono" fontSize={12.5} fill={P.muted}>
          <text x={b.x + 14} y={b.y + 45 + i * 22}>{k}</text>
          <text x={b.x + b.w - 14} y={b.y + 45 + i * 22} textAnchor="end" fill={P.foreground}>{v}</text>
        </g>
      ))}
      {[...ins, ...outs].map((c) => (
        <g key={c.key}>
          <rect x={c.box.x} y={c.box.y} width={c.box.w} height={c.box.h} rx={10} fill={P.surface2} stroke={FG(0.1)} />
          <text x={c.box.x + 14} y={c.box.y + 24} className="font-mono" fontSize={12.5} fill={P.muted}>{short(c.key[0] === "i" ? tx.vin[+c.key.slice(1)]?.prevout?.scriptpubkey_address ?? null : tx.vout[+c.key.slice(1)]?.scriptpubkey_address ?? null)}</text>
          <text x={c.box.x + c.box.w - 14} y={c.box.y + 24} textAnchor="end" className="font-mono" fontSize={10.5} fill={P.faint}>{c.key[0] === "i" ? "IN" : "OUT"} {c.key.slice(1)}</text>
          <text x={c.box.x + 14} y={c.box.y + 49} fontSize={16} fontWeight={500} fill={P.foreground}>{formatBtc(c.key[0] === "i" ? tx.vin[+c.key.slice(1)]?.prevout?.value ?? 0 : tx.vout[+c.key.slice(1)]!.value)}</text>
        </g>
      ))}
      <text x={L.port ? 16 : 24} y={28} className="font-mono" fontSize={11} fontWeight={600} letterSpacing="0.12em" fill={P.faint}>{t("v2.home.lens_wallet_view", { defaultValue: "WALLET VIEW" })}</text>

      {/* Analyst view, clipped to the lens */}
      <g clipPath="url(#lens-clip)">
        <rect width={L.W} height={L.H} fill={P.background} />
        <rect width={L.W} height={L.H} fill={hexToRgba(model.blind ? P.severityGood : COLORS.bitcoin, 0.035)} />
        ...
        {model.blind && !L.port ? (
          <>
            ...
            <text x={L.W / 2} y={L.hub.y - 36} textAnchor="middle" className="font-mono" fontSize={11.5} fontWeight={600} letterSpacing="0.06em" fill={P.foreground}>
        ...
        <circle cx={L.hub.x} cy={L.hub.y} r={L.port ? 12 : 22} fill={P.background} stroke={accent} strokeWidth={2} />
          ...
          {model.hub.map((r, i) => {
            ...
            return (
              <g key={r.kind} className="font-mono">
                ...
                <text x={b.x + (L.port ? 92 : 104)} y={b.y + 45 + i * 22} fontSize={L.port ? 11 : 12} fill={P.foreground}>{v}</text>
              </g>
            );
          })}
        </g>
